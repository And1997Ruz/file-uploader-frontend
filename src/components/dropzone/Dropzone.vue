<template>
  ...
  >
    ...
    <h1>Drag and drop images here or
      <label
        for="file-picker"
        class="choose"
        title="You can choose multiple images at once">Choose a file
      </label>
    </h1>
    <input
      id="file-picker"
    ...
    <div class="file-preview-wrapper" v-if="!!selectedFiles.length">
      <FilePreviewContainer :files="selectedFiles" @removeFile="handleFileRemove"/>
    </div>
    <div class="btn-container" v-if="!!selectedFiles.length">
      <button :style="{ 'cursor': isUploading ? 'not-allowed' : 'pointer'}" @click="handleClear">Clear</button>
      <button :style="{ 'cursor': isUploading ? 'not-allowed' : 'pointer'}" @click="handleUpload">
...
const addFiles = (files: File[]) => {
  ...
  selectedFiles.value = selectedFiles.value.concat(
      filteredFiles.filter(item => !selectedFiles.value.some(i => i.name === item.name && i.size === item.size))
  );
}
...
const filterSupportedFormat = (files: File[]) => {
  ...
  const filteredFiles = files.filter(file => supportedFormats.includes(file.type));

  if(filteredFiles.length !== files.length){
    alertStore.setAlert("error", "One or more files were rejected. Supported formats are: [jpg, jpeg, png]")
  }
